...
define(['knockout', 'text!templates/section-cors.html', 'text!json/svg-headers.json'], function (ko, htmlString, svgJson) {
  ...
  Cors.prototype.submitCorsRequest = function () {
    ...
    self.waitDotInterval = window.setInterval(function () {
      if (self.waitDots() < 3) self.waitDots(self.waitDots() + 1);else self.waitDots(0);
    }, 300);
    self.posting(true);
    ...
    ajax.onreadystatechange = function () {
      ...
      if (this.readyState === 4 && this.status === 200) {
        ...
        if (response.hasErrors) {
          response.globalErrors.forEach(function (ge) {
            return self.errors.push({
              field: 'global',
              message: ge
            });
          });
          Object.keys(response.fieldErrors).forEach(function (k) {
            return self.errors.push({
              field: k,
              message: response.fieldErrors[k]
            });
          });
          return;
        }
  ...
  Cors.prototype.scrollToCorsConfirmation = function () {
