...
public class RequestSoftwareForm {
    ...
    @Pattern(regexp="[A-Z][A-Za-z'\\s\\.\\-]*\\s*", message="{validate.productRequest.invalidFirstName}")
    private String firstName;

    @NotBlank(message="{validate.productRequest.noLastName}")
    @Pattern(regexp="[A-Z][A-Za-z'\\s\\.\\-]*\\s*", message="{validate.productRequest.invalidLastName}")
