define(['knockout', 'text!templates/sample-test.html', 'text!json/svg-headers.json'], function (ko, htmlString, headerJson) {
    ...
    function SampleTest(params) {
        ...
        document.body.addEventListener("loadSampleTestImages", self.eventHandler, {capture: false, once: true, passive: true});
        self.tLastPresentation = -1;
    }

    SampleTest.Count = 0;

    SampleTest.prototype.loadImages = function () {
        var self = this;
    ...
    SampleTest.prototype.koDescendantsComplete = function (node) {
        var self = this;
        self.canvas = document.getElementById(self.canvasId);
        self.ctx = self.canvas.getContext("2d");
        if (!self.slide.complete) {
            self.slide.onload = function () {
    ...
    SampleTest.prototype.onMouseOver = function () {
        var self = this;
    ...
    SampleTest.prototype.onMouseOut = function () {
        var self = this;
        self.isMouseOver = false;
        if (self.ctx !== undefined)
            self.ctx.drawImage(self.slide, 0, 0, self.canvas.width, self.canvas.height);
    ...
    SampleTest.prototype.slideImageLoad = function () {
        var self = this;
        self.slideImage = document.getElementById(self.slideImgId);
    };

    SampleTest.prototype.playPresentation = function (t) {
        var self = this;
        if (!self.isMouseOver) {
            self.tLastPresentation = -1;
            return;
